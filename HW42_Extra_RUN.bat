...
::CALL mvn clean package -Dyc.version="%APP_VERSION%" -Dyc.mainClass="%MAIN_CLASS%" > report_HW42-extra-%APP_VERSION%-%MAIN_CLASS%-Maven.txt
CALL mvn clean package -Dyc.version="%APP_VERSION%" > report_HW42-Extra-%REPO_NAME%-%APP_VERSION%-Maven.txt
...
::java -cp C:\%WS_DIR%\%REPO_NAME%\target\Fat-%APP_VERSION%.jar %MAIN_CLASS_01% -h >> report_HW42-Extra-String-%APP_VERSION%-%MAIN_CLASS_01%-Runner.txt
...
java -cp C:\%WS_DIR%\%REPO_NAME%\target\Fat-%APP_VERSION%.jar %MAIN_CLASS_02% -h >> report_HW42-Extra-String-%APP_VERSION%-%MAIN_CLASS_02%-Runner.txt
java -cp C:\%WS_DIR%\%REPO_NAME%\target\Fat-%APP_VERSION%.jar %MAIN_CLASS_02% -c TC-002.01 -u https://www.wikipedia.org -t Wikipedia >> report_HW42-Extra-String-%APP_VERSION%-%MAIN_CLASS_02%-Runner.txt
java -cp C:\%WS_DIR%\%REPO_NAME%\target\Fat-%APP_VERSION%.jar %MAIN_CLASS_02% -c TC-002.02 -u https://www.wikipedia.org -t abc >> report_HW42-Extra-String-%APP_VERSION%-%MAIN_CLASS_02%-Runner.txt


ECHO List - Command Line Options
ECHO Executing Java programm Selenium Web driver...
::java -cp C:\%WS_DIR%\%REPO_NAME%\target\Fat-%APP_VERSION%.jar %MAIN_CLASS_03% -h >> report_HW42-Extra-List-%APP_VERSION%-%MAIN_CLASS_03%-Runner.txt
::java -cp C:\%WS_DIR%\%REPO_NAME%\target\Fat-%APP_VERSION%.jar %MAIN_CLASS_03% -l "http://www.youtube.com|YouTube" "https://www.wikipedia.org|Wikipedia" "http://git-scm.com|Git" "http://www.apple.com|Apple" "https://www.google.com|Google" "https://www.ups.com/index.html|Delivery Service from UPS" "https://rexter.com|Rexter" "http://www.youtube.com|A" "https://www.wikipedia.org|B" "http://git-scm.com|C" "http://www.apple.com|D" "https://www.google.com|E" "https://www.ups.com/index.html|F" "https://rexter.com|G" >> report_HW42-Extra-List-%APP_VERSION%-%MAIN_CLASS_03%-Runner.txt
